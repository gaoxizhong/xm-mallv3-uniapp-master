		...
		<view :class="{'tui-order-list':scrollTop>=0}">
			<view class="tui-order-item" v-for="(item,orderIndex) in orderList" :key="orderIndex">
				...
				<view class="tui-order-btn">
					...
					<view v-if="item.order_status_id==3" class="tui-btn-ml">
	...
	export default {
		...
		methods: {
			...
			toPayTap: function(total, orderid) {
				const redirectUrl = "/pages/my/myOrder/myOrder?ptype=" + this.ptype;
			...
			addEvaluate(orderid) {
				this.tui.href('/pages/my/addEvaluate/addEvaluate?id=' + orderid)
			}
		},
		onPullDownRefresh() {
			setTimeout(() => {
