		...
		<view class="tui-content-box">
			...
			<view class="tui-box tui-order-box" style="margin-top: 20rpx;">
				<tui-list-cell :arrow="true" padding="0" unlined :lineLeft="false"
					@click="navigateTo('/pages/my/adminstore/order?ptype=2')">
				...
				<view class="tui-order-list">
					<view class="tui-order-item" @tap="navigateTo('/pages/my/publicOrder/order?identity=store')">
						<view class="tui-icon-box">
							<tui-icon name="clock" :size="30" color="#ff0000"></tui-icon>
						</view>
						<view class="tui-order-text">接单广场</view>
					</view>
					<view class="tui-order-item" @tap="navigateTo('/pages/my/adminstore/order?currentTab=2&ptype=2')">
						<view class="tui-icon-box">
							<tui-icon name="signin" :size="30" color="#666666"></tui-icon>
						</view>
						<view class="tui-order-text">已接单</view>
					</view>
					<view class="tui-order-item" @tap="navigateTo('/pages/my/adminstore/order?currentTab=3&ptype=2')">
						<view class="tui-icon-box">
							<tui-icon name="feedback" :size="30" color="#666666"></tui-icon>
						</view>
						<view class="tui-order-text">服务中</view>
					</view>
					<view class="tui-order-item" @tap="navigateTo('/pages/my/adminstore/order?currentTab=5&ptype=2')">
						<view class="tui-icon-box">
			...
			<view class="tui-box tui-tool-box">
				<tui-list-view>
					<tui-list-cell @tap="navigateTo('/pages/my/publicOrder/order?identity=store')" :arrow="true">
						<view class="tui-item-box">
							<tui-icon name="clock-fill" :size="24" color="#ff0000"></tui-icon>
							<view class="tui-list-cell_name">我要接单</view>
						</view>
					</tui-list-cell>
					<tui-list-cell v-if="detail.is_audit!=1" @tap="navigateTo('/pages/my/withdraw/withdraw?mo=store')"
						:arrow="true">
						<view class="tui-item-box">
					...
					<tui-list-cell @tap="navigateTo('/pages/my/withdraw/withdrawlog?mo=store')" :arrow="true">
						<view class="tui-item-box">
							<tui-icon name="listview" :size="24" color="#5677fc"></tui-icon>
							<view class="tui-list-cell_name">提现记录</view>
						</view>
					</tui-list-cell>
					<tui-list-cell v-if="detail.is_audit!=1"
						@tap="navigateTo('/pages/my/memberbankcard/memberbankcard')" :arrow="true">
						<view class="tui-item-box">
					...
					<tui-list-cell @tap="navigateTo('/pages/my/adminstore/update')" :arrow="true">
						<view class="tui-item-box">
							<tui-icon name="setup" :size="24" color="#5677fc"></tui-icon>
...
<script>
	export default {
		...
		onShow() {
			let _this = this;
			_this.$request.get('adminstore.detail').then(res => {
				if (res.errno == 0) {
					if (res.data == 0) {
						uni.showModal({
							title: '提示',
							...
							success: function(res) {
								if (res.cancel) { //点击取消,默认隐藏弹框
									...
									uni.reLaunch({
										url: '/pages/login/login?ptype=store'
									});
								}
							}
						});
