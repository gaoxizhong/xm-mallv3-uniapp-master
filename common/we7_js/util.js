...
util.url = function(action, querystring) {
	...
	var url = app.siteInfo.siteroot + '?i=' + app.siteInfo.uniacid + '&t=' + app.siteInfo.multiid + '&v=' + app
		.siteInfo.version + '&from=wxapp&';
	...
	if (urlquery.length > 0) {
		...
		if (urli) {
			url = 'https://' + window.location.host + '/app/index.php?t=' + app.siteInfo.multiid + '&v=' + app
				.siteInfo
				.version + '&from=wxapp&' + urli + '&';
		}
		console.log(url);
	}
	//#endif
		...
		util.getWe7User(function(userInfo) {
			...
			if (isWeixin) {
				window.location.href =
					'/addons/xm_mallv3/samos/public/index.php?s=/index/wechatmp/wechat&xmtoken=' +
					userInfo.sessionid +
					'&i=' + App.siteInfo.uniacid + '&backurl=' + encodeURIComponent(location.href)
			}
			typeof cb == "function" && cb(userInfo);
		}, '')
...
util.getmpuserinfo = function() {
	util.getUserInfo(function(userInfo) {
		...
		if (isWeixin) {
			window.location.href =
				'/addons/xm_mallv3/samos/public/index.php?s=/index/wechatmp/wechatuserinfo&xmtoken=' +
				userInfo.sessionid +
				'&i=' + App.siteInfo.uniacid + '&backurl=' + encodeURIComponent(location.href)
		}
	})
